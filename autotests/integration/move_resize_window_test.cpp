...
void MoveResizeWindowTest::testRestrictedMove_data()
{
    ...
    // window is initially at (500, 500, 100, 100 + titleThickness)
    QTest::newRow("push down") << false << QPoint(0, -501) << QPoint(500, 0) << false;
    QTest::newRow("push up") << false << QPoint(0, 530) << QPoint(500, 1024) << true;
    QTest::newRow("push left") << false << QPoint(700, 0) << QPoint(1180, 500) << false;
    QTest::newRow("push right") << false << QPoint(-520, 0) << QPoint(0, 500) << false;
    ...
    // *                  *
    // *----**********----*
    QTest::newRow("push down with struts") << true << QPoint(0, -410) << QPoint(500, 100) << false;
    QTest::newRow("push up with struts") << true << QPoint(200, 770) << QPoint(700, 924) << true;
    QTest::newRow("push left with struts") << true << QPoint(600, 400) << QPoint(1080, 900) << false;
    QTest::newRow("push right with struts") << true << QPoint(-410, 0) << QPoint(100, 500) << false;
}
...
void MoveResizeWindowTest::testRestrictedMove()
{
    ...
    QVERIFY(decorationLeft.isEmpty());
    QVERIFY(decorationRight.isEmpty());
    QVERIFY(!decorationTop.isEmpty());
    QVERIFY(decorationBottom.isEmpty());
...
void MoveResizeWindowTest::testRestrictedResizeRight()
{
    ...
    MOTION(QPoint(window->frameGeometry().right(), window->frameGeometry().top()));
    PRESS;
    QVERIFY(!window->isInteractiveResize());
    QVERIFY(interactiveMoveResizeStartedSpy.wait());
    QVERIFY(window->isInteractiveResize());
