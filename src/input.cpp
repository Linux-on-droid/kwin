...
public:
    ...
    bool touchDown(qint32 id, const QPointF &pos, std::chrono::microseconds time) override
    {
        ...
        if (touchSurfaceAllowed(pos)) {
            Window *window = input()->findToplevel(pos);
            seat->notifyTouchDown(window->surface(), window->pos(), id, pos);
        }
        return true;
    }
...
private:
    ...
    bool touchSurfaceAllowed(const QPointF &pos) const
    {
        Window *window = input()->findToplevel(pos);
        if (!window) {
            return true;
        }
        return surfaceAllowed(window->surface());
    }
};
...
public:
    ...
    bool touchDown(qint32 id, const QPointF &pos, std::chrono::microseconds time) override
    {
        ...
        seat->notifyTouchDown(w->surface(), w->framePosToClientPos(w->pos()), id, pos);
        seat->setTimestamp(time);
        QObject::connect(
            w, &Window::frameGeometryChanged, seat, [w]() {
                auto seat = waylandServer()->seat();
                const QPointF pos = w->framePosToClientPos(w->pos());
                seat->setTouchSurfacePosition(w->surface(), -1 * w->inputTransformation().map(pos) + pos);
            },
            Qt::UniqueConnection);
        auto surface = w->surface();
        QObject::connect(
            surface, &SurfaceInterface::aboutToBeDestroyed, waylandServer(), [surface]() {
                auto seat = waylandServer()->seat();
                seat->cancelSurface(surface);
            },
            Qt::UniqueConnection);
        return true;
    }
