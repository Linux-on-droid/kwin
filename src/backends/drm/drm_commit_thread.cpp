...
DrmCommitThread::DrmCommitThread(DrmGpu *gpu, const QString &name)
{
    ...
    m_thread.reset(QThread::create([this]() {
        ...
        while (true) {
            ...
            if (!m_commits.front()->areBuffersReadable() || (m_commits.front()->targetPageflipTime() && *m_commits.front()->targetPageflipTime() > m_targetPageflipTime + s_pageflipSlop)) {
...
void DrmCommitThread::optimizeCommits(TimePoint pageflipTarget)
{
    ...
    const auto targetWithSlop = pageflipTarget + s_pageflipSlop;
    ...
    if (m_commits.front()->areBuffersReadable()) {
        const auto firstNotReady = std::find_if(m_commits.begin() + 1, m_commits.end(), [targetWithSlop](const auto &commit) {
            return !commit->areBuffersReadable() || (commit->targetPageflipTime().has_value() && *commit->targetPageflipTime() > targetWithSlop);
        });
    ...
    for (auto it = m_commits.begin(); it != m_commits.end();) {
        ...
        const auto firstNotSamePlaneNotReady = std::find_if(startIt + 1, m_commits.end(), [&startCommit, targetWithSlop](const auto &commit) {
            return startCommit->modifiedPlanes() != commit->modifiedPlanes() || !commit->areBuffersReadable() || (commit->targetPageflipTime().has_value() && *commit->targetPageflipTime() > targetWithSlop);
        });
    ...
    if (m_commits.front()->areBuffersReadable() && (!m_commits.front()->targetPageflipTime() || *m_commits.front()->targetPageflipTime() <= targetWithSlop)) {
    ...
    for (auto it = m_commits.begin() + 1; it != m_commits.end();) {
        auto &commit = *it;
        if ((commit->targetPageflipTime() && *commit->targetPageflipTime() > targetWithSlop) || !commit->areBuffersReadable()) {
            // this commit isn't ready yet
            it++;
            continue;
        }
